...
bool BufferPoolManager::find_victim_page(frame_id_t* frame_id) {
    ...
    // 情况2: 缓冲池中无空闲帧，使用replacer选择victim
...
Page* BufferPoolManager::new_page(PageId* page_id) {
    ...
    // 3. 如果victim页面是脏页，将其写回磁盘
    if (new_page->is_dirty_) {
        disk_manager_->write_page(new_page->get_page_id().fd,
                                new_page->get_page_id().page_no,
                                new_page->get_data(), PAGE_SIZE);
        new_page->is_dirty_ = false;
    }

    // 从page_table_中移除旧页面
    if (new_page->get_page_id().page_no != INVALID_PAGE_ID) {
        page_table_.erase(new_page->get_page_id());
    }

    // 2. 分配一个新的page_id（需要指定fd）
    PageId new_page_id;
    new_page_id.fd = page_id->fd;  // 使用传入的fd
